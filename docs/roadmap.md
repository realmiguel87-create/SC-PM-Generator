...
- [x] **VERIFIED: a real interactive sign-in, end to end, in a real browser.** A user signed in against the live Stirling Entra ID tenant, the UI showed their account, and a real access token reached the API and was accepted by `Microsoft.Identity.Web`'s JWT bearer validation. Confirmed by the API returning `500` rather than `401` — the request passed token validation and the authorization policy, reached the handler, and failed only at the database (unrelated: the developer's network blocks outbound port 1433). A rejected token would have produced `401`; getting past it to a database error is the proof.
...
- [ ] **Remaining gap: recurring jobs are not registered if startup fails this way, and will not be until the app is restarted against a reachable database.** Scheduled snapshots would silently not run in the meantime. That is a real operational risk for a platform where snapshots feed committee reporting, and the honest fix is a background retry (an `IHostedService` that keeps attempting registration until it succeeds) rather than the current restart-required behaviour. Not implemented — deliberately left as a stated gap rather than a silent one.
