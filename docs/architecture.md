...
See [`docs/disaster-recovery-runbook.md`](disaster-recovery-runbook.md) for backup mechanisms, RPO/RTO targets, and step-by-step recovery procedures per failure scenario — written directly against `infrastructure/bicep/main.bicep`'s actual provisioned resources, including an explicit list of what a production DR posture would still need that the current IaC doesn't yet provide (blob soft delete, Key Vault purge protection, SQL geo-failover, a secondary App Service region).
