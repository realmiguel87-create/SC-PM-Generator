...
- [ ] **Not yet verified: an actual interactive sign-in through a real browser.** Everything above proves the wiring is structurally correct and nothing broke, but no one has clicked "Sign in", completed a real Microsoft login popup, and confirmed a real access token reaches the API and is accepted by `Microsoft.Identity.Web`'s JWT bearer validation. The scope name is assumed to be `access_as_user` exactly as suggested during setup — if the real app registration used a different name, `loginRequest` will request a scope that doesn't exist and login will fail with an `invalid_scope` error until `VITE_MSAL_API_SCOPE` is corrected.
